# ADR 0003: Parser dispatch and RELP native boundaries
...
- **RELP transport**: how the agent forwards already parsed and filtered records
  to the server.

They should not be coupled. A SWIG wrapper over `librelp` can only help the RELP
...
but it does not replace RELP delivery, TLS, retry, or durable buffering.

The current repository already uses a managed RELP transport adapter over the
`DeltaZulu.Relp` submodule for forwarding, while parser code is owned by input
...
Keep RELP transport behind `IDeliveryTransport`. Revisit `librelp` only if RELP
wire-compatibility, TLS behavior, throughput, or protocol maintenance evidence
shows the managed `DeltaZulu.Relp` adapter is insufficient.

## Rationale

### Why managed parser dispatch instead of SWIG/librelp?

Parser dispatch and RELP are different layers. Netlogon, IIS, DNS, auditd, and
...
dictionaries. A native RELP binding does not address those requirements.
...
measurements prove the managed RELP path is inadequate, a SWIG dependency would
...
- Benchmark managed RELP and parser dispatch separately; evaluate `librelp`
