...
internal sealed class BackpressureController
{
    ...
    public BackpressureController(DeltaZuluBufferOptions options) => _options = options;
        ...
        int retryQueueDepth)
    {
        ...
        double diskRatio = _options.MaxDiskBytes > 0
            ? (double)diskBytesUsed / _options.MaxDiskBytes
            : 0;
        double memRatio = _options.MaxMemoryBytes > 0
