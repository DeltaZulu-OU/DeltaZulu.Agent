...
public sealed class ResourceProfileValidator
{
    public IReadOnlyList<string> Validate(ResourceProfile profile)
    {
        ...
        if (profile.SchemaVersion <= 0) errors.Add("schemaVersion must be greater than zero.");
        if (string.IsNullOrWhiteSpace(profile.Id)) errors.Add("id is required.");
        if (string.IsNullOrWhiteSpace(profile.Name)) errors.Add("name is required.");
        if (string.IsNullOrWhiteSpace(profile.Version)) errors.Add("version is required.");
        if (string.IsNullOrWhiteSpace(profile.Resource.Platform)) errors.Add("resource.platform is required.");
        if (string.IsNullOrWhiteSpace(profile.Resource.Family)) errors.Add("resource.family is required.");
